...
for path in Path(folder_selected).rglob('*'):
    ...
    if path.is_file():
        f = open(path, "r")
        inputText = f.read()
        file_match_ents = get_validator_matches(inputText)
        all_match_ents.append({"text": inputText, "ents": file_match_ents.copy(), "title": str(path)})
...
displacy.serve(all_match_ents, style="ent", manual=True, options = options)
